...
print(f"  元:     {t1:.2f}s  →  最適化: {t2:.2f}s  ({t1/t2:.1f}x)  diff={diff:.2e}")
results.append(('Limiter', t1, t2, t1/t2, diff))
...
print(f"  元:     {t1:.2f}s  →  最適化: {t2:.2f}s  ({t1/t2:.1f}x)  diff={diff:.2e}")
results.append(('Phase Limit', t1, t2, t1/t2, diff))
...
print(f"  元:     {t1:.2f}s  →  最適化: {t2:.2f}s  ({t1/t2:.1f}x)  diff={diff:.2e}")
results.append(('Multiband', t1, t2, t1/t2, diff))
...
print(f"  元:     {t1:.2f}s  →  最適化: {t2:.2f}s  ({t1/t2:.1f}x)  diff={diff:.2e}")
results.append(('Loudness Map', t1, t2, t1/t2, diff))
...
print(f"  元:     {t1:.2f}s  →  最適化: {t2:.2f}s  ({t1/t2:.1f}x)  diff={diff:.2e}")
results.append(('Single Band', t1, t2, t1/t2, diff))
...
for name, t_orig, t_fast, spd, diff in results:
    tag = "✓" if diff < 1e-6 else ("~" if diff < 0.01 else "✗")
    print(f"  {tag} {name:18s}: {t_orig:7.2f}s → {t_fast:7.2f}s  ({spd:5.1f}x)  diff={diff:.2e}")
print(f"\n  合計: {total_orig:.2f}s → {total_fast:.2f}s  ({total_orig/total_fast:.1f}x)")
print("\n完了！ (=^･ω･^=)")
